...
const LoginPage = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (mode === 'signin') {
      const isAdmin = await staffLogin(email, password, 'admin');
      if (isAdmin) { navigate('/admin'); return; }
      const isChef = await staffLogin(email, password, 'chef');
          ...
          <div className="bg-card rounded-2xl border border-border shadow-elevated p-8">
            ...
            <div className="flex p-1 bg-muted/50 rounded-xl mb-6">
              <button
                ...
                className={`flex-1 flex items-center justify-center gap-2 py-2.5 rounded-lg text-sm font-bold transition-all ${mode === 'signin'
              ...
              <button
                ...
                className={`flex-1 flex items-center justify-center gap-2 py-2.5 rounded-lg text-sm font-bold transition-all ${mode === 'signup'
                    ? 'bg-card text-foreground shadow-sm'
                    : 'text-muted-foreground hover:text-foreground'
                  }`}
              >
