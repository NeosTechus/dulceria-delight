  ...
  const highlights = [
    {
      ...
      gradient: 'from-fiesta-pink/20 to-fiesta-pink/5',
      iconBg: 'bg-fiesta-pink/15 text-fiesta-pink',
      link: '/candy-shop',
    },
    {
      ...
      gradient: 'from-fiesta-orange/20 to-fiesta-orange/5',
      iconBg: 'bg-fiesta-orange/15 text-fiesta-orange',
      link: '/menu',
    },
    {
      icon: PartyPopper,
      ...
      gradient: 'from-fiesta-turquoise/20 to-fiesta-turquoise/5',
      iconBg: 'bg-fiesta-turquoise/15 text-fiesta-turquoise',
      link: '/candy-shop',
    },
  ];
    ...
    <>
          ...
          <div className="grid md:grid-cols-3 gap-8 max-w-5xl mx-auto">
            {highlights.map((h, i) => (
              <Link
                key={h.title}
                to={h.link}
                className={`group relative bg-gradient-to-br ${h.gradient} rounded-3xl p-8 text-center hover:shadow-fiesta-lg hover:-translate-y-3 transition-all duration-500 border border-border/50 scroll-reveal scroll-reveal-delay-${i + 1}`}
              >
                <div className={`w-20 h-20 rounded-3xl ${h.iconBg} flex items-center justify-center mx-auto mb-6 group-hover:scale-110 transition-transform duration-300`}>
                  <h.icon size={36} />
                </div>
      ...
      <section className="py-24 bg-muted/30 relative papel-picado">
        <div className="container mx-auto px-4">
          <div className="text-center mb-16 scroll-reveal">
            <span className="inline-block text-sm font-extrabold text-accent uppercase tracking-[0.2em] mb-4">
              {t('featured.subtitle')}
            </span>
            <h2 className="text-4xl md:text-6xl font-fredoka text-foreground mb-4">
              {t('featured.title')}
          ...
          <div className="grid md:grid-cols-3 gap-6 max-w-6xl mx-auto">
            {featuredPhotos.map((photo, i) => (
              <Link
                key={photo.label}
                to={photo.link}
                className={`group relative overflow-hidden rounded-3xl aspect-[3/4] shadow-elevated hover:shadow-fiesta-lg transition-all duration-500 scroll-reveal scroll-reveal-delay-${i + 1}`}
              >
                <img
                  src={photo.src}
                  alt={photo.label}
      ...
      <section className="py-24 bg-background relative overflow-hidden">
        ...
        <div className="container mx-auto px-4 relative">
          <div className="text-center mb-16 scroll-reveal">
            <span className="inline-block text-sm font-extrabold text-fiesta-pink uppercase tracking-[0.2em] mb-4">
          ...
          <div className="grid md:grid-cols-3 gap-8 max-w-5xl mx-auto">
            {testimonials.map((review, i) => (
              <div
                key={i}
                className={`relative bg-card rounded-3xl border border-border/50 p-8 hover:shadow-fiesta-lg hover:-translate-y-2 transition-all duration-500 scroll-reveal scroll-reveal-delay-${i + 1}`}
              >
      ...
      {/* CTA Banner */}
      <section className="py-20 bg-gradient-fiesta relative overflow-hidden">
        ...
        <div className="container mx-auto px-4 text-center relative">
          <h2 className="text-4xl md:text-5xl font-fredoka text-primary-foreground mb-5 drop-shadow-lg">
            {t('cta.title')}
          </h2>
          ...
          <Link
            to="/menu"
            className="inline-block bg-primary-foreground text-foreground font-bold text-lg px-10 py-5 rounded-full hover:scale-105 transition-all duration-300 shadow-[0_8px_30px_-5px_rgba(255,255,255,0.3)] hover:shadow-[0_12px_40px_-5px_rgba(255,255,255,0.4)]"
          >
            {t('cta.button')}
          </Link>
        </div>
