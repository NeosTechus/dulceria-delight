      ...
      <div className="relative bg-card rounded-2xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
        {step === 'success' ? (
          <div className="p-10 text-center">
            ...
            <p className="text-muted-foreground">Your order has been placed. We'll have it ready for pickup!</p>
          ...
          <>
            ...
            <div className="p-5 border-b border-border bg-muted/30">
              <h4 className="font-bold text-foreground mb-3">Order Summary</h4>
              {items.map((item) => (
            ...
            <form onSubmit={handleSubmit} className="p-5 space-y-4">
              ...
              <div>
                <label className="block text-sm font-bold text-foreground mb-1">Card Number</label>
                <input
                  required
                  type="text"
              ...
              <button
                type="submit"
                className="w-full bg-gradient-fiesta text-primary-foreground font-bold py-4 rounded-xl text-lg shadow-fiesta hover:scale-[1.02] transition-transform mt-2"
              >
                Pay ${(total + tax).toFixed(2)}
              </button>
              <p className="text-xs text-muted-foreground text-center">🔒 Demo only — no real payment processed</p>
            </form>
          </>
        )}
      </div>
