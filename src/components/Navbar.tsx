          ...
          <div className="hidden md:flex items-center gap-6">
            {navLinks.map(({ label, to }) => (
              <Link
                key={to}
                to={to}
                className={`font-nunito font-bold transition-all duration-200 relative py-1 ${location.pathname === to ? 'text-primary' : 'text-foreground/70 hover:text-primary'
                  }`}
              >
                {label}
            ...
            <Link
              to="/orders"
              className={`font-nunito font-bold transition-all duration-200 relative py-1 flex items-center gap-1.5 ${location.pathname === '/orders' ? 'text-primary' : 'text-foreground/70 hover:text-primary'
                }`}
            >
              <Package size={16} />
              Orders
            ...
            {isAuthenticated ? (
              <motion.div className="flex items-center gap-3">
                <Link
                  to={user?.role === 'chef' ? '/chef' : user?.role === 'admin' ? '/admin' : '/orders'}
            ...
            >
              {navLinks.map(({ label, to }, i) => (
                ...
                >
                  <Link
                    ...
                    className={`font-bold text-left py-3 px-4 rounded-xl transition-all block ${location.pathname === to ? 'text-primary bg-primary/5' : 'text-foreground/70 hover:bg-muted'
                      }`}
                  >
                    {label}
                  </Link>
              ...
              >
                <Link
                  to="/orders"
                  onClick={() => setMobileOpen(false)}
                  className={`font-bold text-left py-3 px-4 rounded-xl transition-all flex items-center gap-2 ${location.pathname === '/orders' ? 'text-primary bg-primary/5' : 'text-foreground/70 hover:bg-muted'
                    }`}
                >
